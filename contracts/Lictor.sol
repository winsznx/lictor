...
/**
 * @title Lictor
 * @notice Set a thesis. Agents watch the markets. When your conditions fire, the trade executes —
 *         before you could have reacted.
 *
 * @dev Full agentic pipeline:
 *      1. LLM inferString  — decomposes thesis into Signal[]
 *      2. JSON API / Parse Website — monitors each signal on each tick
 *      3. LLM inferToolsChat — dispatches when all signals fire; yields ABI-encoded swap calldata
 *
 *      The yielded calldata is validated against the mandate's token allowlist, token pair,
 *      amount ceiling, and slippage floor before the Algebra Integral swap executes.
 */
contract Lictor {
    // ─── Enums ────────────────────────────────────────────────────────────────
